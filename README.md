...
> **输入方式**：在飞书中建一个只有你和 bot 的群聊，平时向这个群发消息即可。飞书个人版/小团队版中，P2P bot 对话框无法直接输入，群聊是更可靠的方案。
...
> **注意**：三个权限均需开通。权限开通后必须重新**发布应用**才能生效。
...
| | 当前手动方案 | 本自动化方案 |
|--|--|--|
| 操作 | 手动导出 txt → 运行 py | 发完消息等零点自动收集 |
| 依赖 | 无 | 飞书自建应用 + GitHub |
| 费用 | 免费 | 免费 |
| 可靠性 | 手动，容易忘 | 自动，每日 commit 留痕 |
| 扩展性 | 低 | 可在 Actions 中加 LLM 分析等后处理 |
